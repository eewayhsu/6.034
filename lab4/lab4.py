...
#THIS IS BEYOND FRUSTRATING
    ...
    if X is not None:  
        ...
        for constraint in constraints:
            if state.get_variable_by_name(constraint.get_variable_i_name()) is X:
	    ...
	    for value in Y.get_domain():
                if constraint.check(state, x, value) is False:
    ...
    for constraint in constraints:
    	...
    	if constraint.get_variable_i_name() is X.get_name():
	
	   FAILED ATTEMPT TO SIMPLIFY.  
	   	     Y_name = constraint.get_variable_j_name()
		else:
	   	     Y_name = constraint.get_variable_i_name()
		...
		Y = state.get_variable_by_name(Y_name)
		for value in Y.get_domain():
	    	    if constraint.check(state, value, x) is False:
	        	Y.reduce_domain(value)
	    	    if Y.domain_size is 0:
			return False
    	return True
	    ...
	    Y = state.get_variable_by_name(constraint.get_variable_j_name())
            for value in Y.get_domain():
                if constraint.check(state, x, value) == False:
                    Y.reduce_domain(value)
                if Y.domain_size() is 0:
                    return False
	elif constraint.get_variable_j_name() is X.get_name():
            Y = state.get_variable_by_name(constraint.get_variable_i_name())
            for value in Y.get_domain():
                 if constraint.check(state, value, x) == False:
                    Y.reduce_domain(value)
                 if Y.domain_size() is 0:
                    return False
    return True"""    			
...
def forward_checking_prop_singleton(state, verbose=False):
	...
	x = X.get_domain()[0]
		...
		Y = state.get_variable_by_name(constraint.get_variable_j_name())
                for value in Y.get_domain():
                    if not constraint.check(state, value, x):
                        Y.reduce_domain(value)
                    if Y.domain_size() is 0:
                        return False	    
    	...
    	for variable in state.get_all_variables():
            if variable.domain_size() == 1 and variable not in visited and variable not in singletons:
            	singletons.append(variable)
